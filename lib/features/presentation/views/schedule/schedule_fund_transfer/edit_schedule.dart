import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:intl/intl.dart';
import 'package:phosphor_flutter/phosphor_flutter.dart';
import 'package:union_bank_mobile/core/theme/theme_data.dart';
import 'package:union_bank_mobile/features/presentation/bloc/base_bloc.dart';
import 'package:union_bank_mobile/features/presentation/bloc/base_event.dart';
import 'package:union_bank_mobile/features/presentation/bloc/base_state.dart';
import 'package:union_bank_mobile/features/presentation/views/fund_transfer/widgets/fund_transfer_data_component.dart';
import 'package:union_bank_mobile/features/presentation/widgets/appbar.dart';
import 'package:union_bank_mobile/features/presentation/widgets/date_pickers/ft_date_picker.dart';
import 'package:union_bank_mobile/utils/app_localizations.dart';

import '../../../../../../core/service/dependency_injection.dart';
import '../../../../../core/theme/text_styles.dart';
import '../../../../../utils/app_sizer.dart';
import '../../../../../utils/enums.dart';
import '../../../../../utils/navigation_routes.dart';
import '../../../../../utils/text_editing_controllers.dart';
import '../../../bloc/fund_transfer_view_scheduling/ft_view_scheduling_bloc.dart';
import '../../../bloc/fund_transfer_view_scheduling/ft_view_scheduling_event.dart';
import '../../../bloc/fund_transfer_view_scheduling/ft_view_scheduling_state.dart';
import '../../../widgets/app_button.dart';
import '../../../widgets/app_switch/app_switch.dart';
import '../../../widgets/pop_scope/ub_pop_scope.dart';
import '../../../widgets/text_fields/app_text_field.dart';
import '../../../widgets/toast_widget/toast_widget.dart';
import '../../base_view.dart';
import '../../fund_transfer/data/fund_transfer_args.dart';

class EditScheduleView extends BaseView {
  final FundTransferArgs fundTransferArgs;

  EditScheduleView({required this.fundTransferArgs});

  @override
  _EditScheduleViewState createState() => _EditScheduleViewState();
}

class _EditScheduleViewState extends BaseViewState<EditScheduleView> {
  var bloc = injection<FTViewSchedulingBloc>();
  int ownRecDays = 0;
  String? startDate;
  String? endDate;
  bool isEdited = false;
  bool? notifyBenificiary;
  final _formKey = GlobalKey<FormState>();
  bool? isStartDateAvailable;
  CurrencyTextEditingController? amountController;
  TextEditingController? whenController;
TextEditingController? mobileNumberController;
  @override
  void initState() {
    super.initState();
     amountController = CurrencyTextEditingController(initialValue: widget.fundTransferArgs.fundTransferEntity.amount!);
    whenController = TextEditingController(text: widget.fundTransferArgs.fundTransferEntity.scheduleType?.toUpperCase() == "REPEAT" ? "Recurring" : "Later");
    mobileNumberController = TextEditingController(text: widget.fundTransferArgs.fundTransferEntity.beneficiaryMobile ?? "");
    ownRecDays =DateTime.parse(
      DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy')
          .parse(widget.fundTransferArgs.fundTransferEntity.startDate!)),
    ).difference(DateTime.now()).inDays;
    startDate = widget.fundTransferArgs.fundTransferEntity.startDate;
    endDate = widget.fundTransferArgs.fundTransferEntity.endDate;
    notifyBenificiary =
        widget.fundTransferArgs.fundTransferEntity.beneficiaryMobile == null
            ? false
            : true;
    isStartDateAvailable =
        widget.fundTransferArgs.fundTransferEntity.tabID == 0 ? false : true;
  }

  @override
  Widget buildView(BuildContext context) {
    return UBPopScope(
      onPopInvoked: () async {
        _onBack();
        widget.fundTransferArgs.fundTransferEntity.startDate = startDate;
        widget.fundTransferArgs.fundTransferEntity.endDate = endDate;
        return true;
      },
      child: Scaffold(
        backgroundColor: colors(context).primaryColor50,
        appBar: UBAppBar(
          title: AppLocalizations.of(context)
              .translate("edit_scheduled_fund_transfer"),
          onBackPressed: () {
            _onBack();
            widget.fundTransferArgs.fundTransferEntity.startDate = startDate;
            widget.fundTransferArgs.fundTransferEntity.endDate = endDate;
          },
        ),
        body: BlocProvider(
          create: (_) => bloc,
          child: BlocListener<FTViewSchedulingBloc,
              BaseState<FTViewSchedulingState>>(
            bloc: bloc,
            listener: (_, state) {
              if (state is EditSchedulingFTSuccessState) {
                ToastUtils.showCustomToast(
                    context, state.responseDes!, ToastStatus.SUCCESS);
                // Navigator.pop(
                //     context, Routes.kFundTransferSchedulingView);
                Navigator.pushNamedAndRemoveUntil(
                  context,
                  Routes.kFundTransferSchedulingView,
                  (Route<dynamic> route) =>
                      route.settings.name == Routes.kScheduleCategoryListView,
                );
                // Navigator.pushNamedAndRemoveUntil(
                //   context,
                //   Routes.kFundTransferSchedulingView,
                //       (route) => true,
                // );
              }
              if (state is EditSchedulingFTFailState) {
                ToastUtils.showCustomToast(
                    context,
                    state.errorMessage ?? AppLocalizations.of(context).translate("something_went_wrong"),
                    ToastStatus.FAIL);
              }
            },
            child: Form(
              key: _formKey,
              autovalidateMode: AutovalidateMode.onUserInteraction,
              child: Padding(
                padding: EdgeInsets.fromLTRB(20.w, 0.h, 20.w, 20.h + AppSizer.getHomeIndicatorStatus(context)),
                child: Column(
                  children: [
                    Expanded(
                        child: SingleChildScrollView(
                            physics: const ClampingScrollPhysics(),
                            child: Padding(
                              padding: EdgeInsets.only(top: 24.h),
                              child: Column(
                                children: [
                                  Container(
                                    decoration: BoxDecoration(
                                      borderRadius: BorderRadius.circular(8).r,
                                      color: colors(context).whiteColor,
                                    ),
                                    child: Padding(
                                      padding: const EdgeInsets.only(
                                              left: 16.0, right: 16)
                                          .w,
                                      child: Column(
                                        children: [
                                          FTSummeryDataComponent(
                                            title: AppLocalizations.of(context)
                                                .translate("paid_from"),
                                            data: widget
                                                    .fundTransferArgs
                                                    .fundTransferEntity
                                                    .payFromName ??
                                                "-",
                                            subData: widget
                                                    .fundTransferArgs
                                                    .fundTransferEntity
                                                    .payFromNum ??
                                                "-",
                                          ),
                                          FTSummeryDataComponent(
                                            title: AppLocalizations.of(context)
                                                .translate("paid_to"),
                                            data: widget
                                                    .fundTransferArgs
                                                    .fundTransferEntity
                                                    .payToacctname ??
                                                "-",
                                            subData: widget
                                                    .fundTransferArgs
                                                    .fundTransferEntity
                                                    .payToacctnmbr ??
                                                "-",
                                          ),
                                          FTSummeryDataComponent(
                                            title: AppLocalizations.of(context)
                                                .translate("schedule_type"),
                                            data: widget
                                                        .fundTransferArgs
                                                        .fundTransferEntity
                                                        .scheduleType
                                                        ?.toUpperCase() ==
                                                    "ONETIME"
                                                ? "One time"
                                                : widget
                                                        .fundTransferArgs
                                                        .fundTransferEntity
                                                        .scheduleType ??
                                                    "-",
                                          ),
                                          if (widget.fundTransferArgs
                                                  .fundTransferEntity.scheduleType
                                                  ?.toUpperCase() ==
                                              "REPEAT")
                                            FTSummeryDataComponent(
                                              title: AppLocalizations.of(context)
                                                  .translate("frequency"),
                                              data: widget
                                                      .fundTransferArgs
                                                      .fundTransferEntity
                                                      .scheduleFrequency ??
                                                  "-",
                                            ),
                                          FTSummeryDataComponent(
                                            isLastItem: true,
                                            title: AppLocalizations.of(context)
                                                .translate(
                                                    "transaction_category"),
                                            data: widget
                                                    .fundTransferArgs
                                                    .fundTransferEntity
                                                    .transactionCategory ??
                                                "-",
                                          ),
                                        ],
                                      ),
                                    ),
                                  ),
                                  16.verticalSpace,
                                  Container(
                                    decoration: BoxDecoration(
                                      borderRadius: BorderRadius.circular(8).r,
                                      color: colors(context).whiteColor,
                                    ),
                                    child: Padding(
                                      padding: const EdgeInsets.all(16.0).w,
                                      child: Column(
                                        children: [
                                          AppTextField(
                                            isInfoIconVisible: false,
                                            hint: AppLocalizations.of(context)
                                                .translate("amount"),
                                            title: AppLocalizations.of(context)
                                                .translate("amount"),
                                            isCurrency: true,
                                            textCapitalization:
                                                TextCapitalization.none,
                                            controller: amountController,
                                            showCurrencySymbol: true,
                                            inputType: const TextInputType.numberWithOptions(decimal: true),
                                            onTextChanged: (value) {
                                              setState(() {
                                                double parsedValue =
                                                    double.tryParse(
                                                            value.replaceAll(
                                                                ',', '')) ??
                                                        88;
                                                widget
                                                    .fundTransferArgs
                                                    .fundTransferEntity
                                                    .amount = parsedValue;
                                                isEdited = true;
                                              });
                                            },
                                            validator: (value){
                                              if (amountController?.text == "" || amountController?.text == null || amountController?.text == "0.00") {
                                                return AppLocalizations.of(context)
                                                    .translate("mandatory_field_msg");
                                              } else {
                                                return null;
                                              }
                                            },
                                          ),
                                        ],
                                      ),
                                    ),
                                  ),
                                  16.verticalSpace,
                                  Container(
                                    decoration: BoxDecoration(
                                      borderRadius: BorderRadius.circular(8).r,
                                      color: colors(context).whiteColor,
                                    ),
                                    child: Padding(
                                      padding: const EdgeInsets.all(16.0).w,
                                      child: Column(
                                        children: [
                                          AppTextField(
                                            isInfoIconVisible: false,
                                            hint: AppLocalizations.of(context)
                                                .translate("when"),
                                            title: AppLocalizations.of(context)
                                                .translate("when"),
                                            isEnable: false,
                                            controller: whenController,
                                            textCapitalization: TextCapitalization.none,
                                            onTextChanged: (value) {},
                                          ),
                                          24.verticalSpace,
                                          if(widget.fundTransferArgs.fundTransferEntity.scheduleType?.toUpperCase() == "REPEAT")
                                            Column(
                                                children:[
                                                  FTDatePicker(
                                                    labelText: AppLocalizations.of(context).translate("start_date"),
                                                    title: AppLocalizations.of(context).translate("start_date"),
                                                    firstDate:DateTime.parse(
                                                        DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy')
                                                            .parse(widget.fundTransferArgs.fundTransferEntity.startDate!))),
                                                    // DateFormat('dd MMMM yyyy').parse(widget.fundTransferArgs.fundTransferEntity.startDate!),
                                                    initialDate:DateTime.parse(
                                                          DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy')
                                                              .parse(widget.fundTransferArgs.fundTransferEntity.startDate!)),
                                                        ) ,
                                                    text: startDate!=null ?DateFormat('dd-MMM-yyyy').format(DateFormat('d MMMM yyyy').parse(startDate!)) :null ,
                                                    initialValue: DateFormat('dd-MMM-yyyy').format(DateFormat('d MMMM yyyy').parse(widget.fundTransferArgs.fundTransferEntity.startDate!)) ,
                                                    isStartDateSelected: isStartDateAvailable ?? true,
                                                    onChange: (value) {
                                                      if(endDate != null){
                                                        setState(() {
                                                          endDate = null;
                                                        });
                                                      }
                                                      startDate = value;
                                                      setState(() {
                                                        ownRecDays = DateTime.parse(
                                                          DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy')
                                                              .parse(startDate!)),
                                                        ).difference(DateTime.now()).inDays;
                                                      });
                                                      isEdited = true;
                                                    },
                                                  ),
                                                  24.verticalSpace,
                                                  FTDatePicker(
                                                    validator: (value){
                                                      if(endDate == null){
                                                        return AppLocalizations.of(context).translate("mandatory_field_msg_selection");
                                                      }
                                                      else{
                                                        return null;
                                                      }
                                                    },
                                                    labelText: AppLocalizations.of(context).translate("end_date"),
                                                    title: AppLocalizations.of(context).translate("end_date"),
                                                    isStartDateSelected: true,
                                                    firstDate: DateTime.now().add(Duration(days: ownRecDays + 2)),
                                                    initialDate: DateTime.now().add(Duration(days: ownRecDays + 2)),
                                                    initialValue:widget.fundTransferArgs.fundTransferEntity.endDate!=null ? DateFormat('dd-MMM-yyyy').format(DateFormat('d MMMM yyyy').parse(widget.fundTransferArgs.fundTransferEntity.endDate!)):null,
                                                    text: endDate!=null ?DateFormat('dd-MMM-yyyy').format(DateFormat('d MMMM yyyy').parse(endDate!)) :null ,
                                                    // DateFormat('dd MMMM yyyy').format(DateFormat('yyyy-MM-dd HH:mm:ss.SSSZ').parse(widget.fundTransferArgs.fundTransferEntity.endDate!)),
                                                    onChange: (value) {
                                                      endDate = value;
                                                      isEdited = true;
                                                    },
                                                  ),
                                                ]
                                            ),
                                          if(widget.fundTransferArgs.fundTransferEntity.scheduleType?.toUpperCase() == "ONETIME")
                                            FTDatePicker(
                                              labelText: AppLocalizations.of(context).translate("transaction_date"),
                                              title: AppLocalizations.of(context).translate("transaction_date"),
                                              firstDate: DateTime.parse(
                                                        DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy')
                                                            .parse(widget.fundTransferArgs.fundTransferEntity.startDate!))),
                                              // DateFormat('dd MMMM yyyy').parse(widget.fundTransferArgs.fundTransferEntity.startDate!),
                                              initialDate:DateTime.parse(DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy').parse(widget.fundTransferArgs.fundTransferEntity.startDate!))),
                                              initialValue: DateFormat('dd-MMM-yyyy').format(DateFormat('d MMMM yyyy').parse(startDate!)) ,
                                              isStartDateSelected: true,
                                              onChange: (value) {
                                                if (widget
                                                        .fundTransferArgs
                                                        .fundTransferEntity
                                                        .endDate !=
                                                    null) {
                                                  setState(() {
                                                    widget
                                                        .fundTransferArgs
                                                        .fundTransferEntity
                                                        .endDate = null;
                                                  });
                                                }
                                                startDate = value;
                                                setState(() {
                                                  ownRecDays = DateTime.parse(
                                                    DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy')
                                                        .parse(widget.fundTransferArgs.fundTransferEntity.startDate!)),
                                                  ).difference(DateTime.now()).inDays;
                                                });
                                                isEdited = true;
                                              },
                                            ),
                                        ],
                                      ),
                                    ),
                                  ),
                                  16.verticalSpace,
                                  Container(
                                    decoration: BoxDecoration(
                                      borderRadius: BorderRadius.circular(8).r,
                                      color: colors(context).whiteColor,
                                    ),
                                    child: AppSwitch(
                                      title: AppLocalizations.of(context)
                                          .translate("notify_the_beneficiary"),
                                      value: notifyBenificiary ?? false,
                                      onChanged: (value) {
                                        setState(() {
                                          notifyBenificiary = value;
                                        });
                                      },
                                      switchItems: [
                                        AppTextField(
                                          validator: (a){
                                            if(mobileNumberController?.text==""||mobileNumberController?.text==null){
                                              return AppLocalizations.of(context).translate("mandatory_field_msg");
                                            } else{
                                              return null;
                                            }
                                          },
                                          isInfoIconVisible: false,
                                          title: AppLocalizations.of(context).translate("beneficiary_mobile_no"),
                                          hint: AppLocalizations.of(context).translate("Enter_mobile_number"),
                                          isCurrency: false,
                                          maxLength: 10,
                                          controller: mobileNumberController,
                                          inputType: TextInputType.phone,
                                          onTextChanged: (value) {
                                            setState(() {
                                              widget
                                                  .fundTransferArgs
                                                  .fundTransferEntity
                                                  .beneficiaryMobile = value;
                                              isEdited = true;
                                            });
                                          },
                                        ),
                                      ],
                                    ),
                                  ),
                                  16.verticalSpace,
                                  Container(
                                    decoration: BoxDecoration(
                                      borderRadius: BorderRadius.circular(8).r,
                                      color: colors(context).secondaryColor200,
                                    ),
                                    child: Padding(
                                      padding: const EdgeInsets.all(16.0).w,
                                      child: Column(
                                        children: [
                                          Row(
                                            crossAxisAlignment:
                                                CrossAxisAlignment.center,
                                            children: [
                                              PhosphorIcon(PhosphorIcons.info(
                                                  PhosphorIconsStyle.bold)),
                                              8.horizontalSpace,
                                              Text(
                                                  AppLocalizations.of(context)
                                                      .translate("please_note"),
                                                  textAlign: TextAlign.justify,
                                                  style: size14weight700.copyWith(
                                                      color: colors(context)
                                                          .blackColor)),
                                            ],
                                          ),
                                          12.verticalSpace,
                                          Text(
                                              AppLocalizations.of(context)
                                                  .translate(
                                                      "edit_scedule_des_complete"),
                                              textAlign: TextAlign.justify,
                                              style: size14weight400.copyWith(
                                                  color:
                                                      colors(context).greyColor)),
                                        ],
                                      ),
                                    ),
                                  ) ,
                                  16.verticalSpace,
                                ],
                              ),
                            ))),
                    Column(
                      children: [
                        20.verticalSpace,
                        AppButton(
                            buttonText:
                                AppLocalizations.of(context).translate("save"),
                            onTapButton: () {
                              if (_formKey.currentState?.validate() == false) {
                                return;
                              }
                              bloc.add(UpdateScheduleFTEvent(
                                messageType: "scheduleFtReq",
                                scheduleId: widget.fundTransferArgs.fundTransferEntity.scheduleId,
                                endDate:widget.fundTransferArgs.fundTransferEntity.scheduleType!.toUpperCase() == "ONETIME" ? DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy').parse(startDate!)):
                                DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy').parse(endDate!)),
                                amount: widget.fundTransferArgs.fundTransferEntity.amount,
                                startDate: DateFormat('yyyy-MM-dd').format(DateFormat('d MMMM yyyy').parse(startDate!)),
                                beneficiaryMobile: widget.fundTransferArgs.fundTransferEntity.beneficiaryMobile,
                                beneficiaryEmail: widget.fundTransferArgs.fundTransferEntity.beneficiaryEmail,
                              ));
                            }),
                        16.verticalSpace,
                        AppButton(
                          buttonType: ButtonType.OUTLINEENABLED,
                          buttonColor: Colors.transparent,
                          buttonText:
                              AppLocalizations.of(context).translate("cancel"),
                          onTapButton: () {
                            _onBack();
                          },
                        ),
                        AppSizer.verticalSpacing(AppSizer.getHomeIndicatorStatus(context)),
                      ],
                    ),
                  ],
                ),
              ),
            ),
          ),
        ),
      ),
    );
  }

  _onBack() {
    if (isEdited == true) {
      showAppDialog(
          alertType: AlertType.WARNING,
          title: AppLocalizations.of(context).translate("are_you_sure?"),
          message: AppLocalizations.of(context).translate("schedule_edit_back_des"),
          negativeButtonText: AppLocalizations.of(context).translate("no"),
          positiveButtonText: AppLocalizations.of(context).translate("yes"),
          onNegativeCallback: () {},
          onPositiveCallback: () {
            widget.fundTransferArgs.fundTransferEntity.startDate = startDate;
            widget.fundTransferArgs.fundTransferEntity.endDate = endDate;
            Navigator.pushNamedAndRemoveUntil(
              context,
              Routes.kFundTransferSchedulingView,
              (Route<dynamic> route) =>
                  route.settings.name == Routes.kScheduleCategoryListView,
            );
            setState(() {});
          });
    } else {
      Navigator.pop(context);
    }
  }

  @override
  BaseBloc<BaseEvent, BaseState> getBloc() {
    return bloc;
  }
}
